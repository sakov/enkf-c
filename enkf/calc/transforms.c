...
void das_calctransforms(dasystem* das)
{
    ...
    if (das->mode == MODE_ENKF) {
        if (rank == 0)
            nc_createX5(FNAME_X5, nj, ni, das->stride, das->nmem, &ncid_X5, &varid_X5);
        X5j = alloc2d(ni, das->nmem * das->nmem, sizeof(float));
        X5 = alloc2d(das->nmem, das->nmem, sizeof(double));
    } else if (das->mode == MODE_ENOI) {
        if (rank == 0)
            nc_createw(FNAME_W, nj, ni, das->stride, das->nmem, &ncid_w, &varid_w);
     ...
     * main cycle 
     */
    enkf_printf("    main cycle (%d x %d local analyses):\n", nj, ni);
    enkf_flush();
#if defined(MPI)
    ...
    if (rank == 0) {
        ...
        if (nprocesses > 1) {
             ...
             * process is approximately equal. If outputs from each process
             * are collected at master for writing after each iteration, then 
             * it has sense to group the iterations collected at master at
             * each time together so that they take approximately the same
             * time. 
             */
#if defined(SHUFFLE_ROWS)
            shuffle(nj, jpool);
    ...
    for (jj = my_first_iteration; jj <= my_last_iteration; ++jj) {
        ...
        for (ii = 0; ii < ni; ++ii) {
...

            {
                ...
                model_fij2xy(m, (double) i, (double) j, &lon, &lat);
        ...
        if (das->mode == MODE_ENKF) {
            if (rank > 0) {
                 ...
                 * write own results 
                 */
                nc_writeX5(FNAME_X5, ncid_X5, jpool[jj], ni, das->nmem, varid_X5, X5j[0]);
                /*
                ...
                for (r = 1; r < nprocesses; ++r) {
                    ...
                    assert(ierror == MPI_SUCCESS);
                    nc_writeX5(FNAME_X5, ncid_X5, jpool[first_iteration[r] + jj], ni, das->nmem, varid_X5, X5j[0]);
                }
            }
        } else if (das->mode == MODE_ENOI) {
            if (rank > 0) {
                 ...
                 * write own results 
                 */
                nc_writew(FNAME_W, ncid_w, jpool[jj], ni, das->nmem, varid_w, wj[0]);
                /*
                 * collect and write results from slaves 
                 */
                for (r = 1; r < nprocesses; ++r) {
                    ...
                    assert(ierror == MPI_SUCCESS);
                    nc_writew(FNAME_W, ncid_w, jpool[first_iteration[r] + jj], ni, das->nmem, varid_w, wj[0]);
                }
            }
        }
#else
    ...
    if (rank == 0) {
        if (das->mode == MODE_ENKF)
            ncw_close(FNAME_X5, ncid_X5);
        else if (das->mode == MODE_ENOI)
            ncw_close(FNAME_W, ncid_w);
    }
#if defined(MPI)
    MPI_Barrier(MPI_COMM_WORLD);
    enkf_printf("  finished calculating transforms\n");
    enkf_flush();
#endif
    ...
    if (rank > 0) {
        ...
        nc_writediag(FNAME_ENKFSTATS, obs->nobstypes, nj, ni, das->stride, nlobs, dfs, srf, pnlobs, pdfs, psrf);
    }
#endif                          /* MPI */
...
void das_dopointlogs(dasystem* das)
{
    ...
    for (p = 0; p < das->nplogs; ++p) {
        ...
        plog_write(das, i, j, lon, lat, (model_getdepth(m))[j][i], ploc, lobs, lcoeffs, sloc, (ploc == 0) ? NULL : Sloc[0], (das->mode == MODE_ENKF) ? X5[0] : w);
        printf("\n");
