...
/*    ncw_inq_varid(ncid, "RFI_stat", &varid_RFI_stat);
    RFI_stat = alloc2d(nj, ni, sizeof(double));
    ncw_get_var_double(ncid, varid_RFI_stat, RFI_stat[0]);
    ncw_get_att_double(ncid, varid_RFI_stat, "_FillValue", &missval);
    ncw_close(ncid); */

    basename[48] = 0;
    if (!str2int(&basename[46], &day))
        enkf_quit("SMOS reader: could not convert file name \"%s\" to date", fname);
    basename[45] = 0;
    if (!str2int(&basename[43], &month))
        enkf_quit("SMOS reader: could not convert file name \"%s\" to date", fname);
    basename[42] = 0;
    if (!str2int(&basename[38], &year))
        enkf_quit("SMOS reader: could not convert file name \"%s\" to date", fname);
    ...
    for (j = 0; j < (int) nj; ++j) {
        for (i = 0; i < (int) ni; ++i) {
            ...
            o->instrument = st_add_ifabscent(obs->instruments, "SMOS", -1);
            o->id = obs->nobs;
            o->fid = fid;
            o->batch = 0;
            o->value = sss[j][i];
