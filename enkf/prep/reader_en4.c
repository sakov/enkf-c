 ...
 * Created:     20/06/2024
 *
 * Authors:     Yonghong Yin, Pavel Sakov
 *              Bureau of Meteorology
 *
 * Description: Reader for quality controlled in-situ temperature and salinity
 *              profile data from EN4 dataset produced by Met Office Hadley
 *              Centre. Based on reader_mmt.c.
 *
 *              Optional parameters:
 *              - EXCLUDEINST
...
void reader_en4(char* fname, int fid, obsmeta* meta, grid* g, observations* obs)
{
    ...
    ncw_inq_varid(ncid, "DEPH_CORRECTED", &varid);
    ...
    if (varid >= 0) {
        ...
        ncw_get_var_text(ncid, varid, qc[0]);
    }
...
{
    enkf_printf("\n  Reader \"en4\" reads EN4 quality controlled in-situ temperature and\n\
salinity data produced by Met Office Hadley Centre.\n\
\n\
