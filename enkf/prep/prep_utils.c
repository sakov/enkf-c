...
void print_obsstats(observations* obs, observations* sobs)
{
    ...
    enkf_printf("    type    # used     # dropped  # outside  # land     # shallow  # badvalue # superobs.\n");
    enkf_printf("    ----------------------------------------------------------------------------------------\n");
    for (i = 0; i < obs->nobstypes; ++i) {
        obstype* ot = &obs->obstypes[i];

        enkf_printf("    %s     %-10d %-10d %-10d %-10d %-10d %-10d %-10d\n", ot->name, ot->ngood, ot->nobs - ot->ngood, ot->noutside, ot->nland, ot->nshallow, ot->nrange, sobs->obstypes[i].nobs);
    }
    enkf_printf("    total   %-10d %-10d %-10d %-10d %-10d %-10d %-10d\n", obs->ngood, obs->nobs - obs->ngood, obs->noutside, obs->nland, obs->nshallow, obs->nrange, sobs->nobs);
}
