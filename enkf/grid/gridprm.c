 ...
 *
 * Purpose:     Code for reading the grid parameter file.
 *
 * Description:
 *
 * Revisions:
...
int ngridhtypeentries = sizeof(allgridhtypeentries) / sizeof(gridhtype_entry);

/**
 */
void gridprm_create(enkfprm* eprm, int* ngrid, gridprm** prm)
{
    char* fname = eprm->gridprm;
    gridprm* now = NULL;
    FILE* f = NULL;
    int ignore = 0;
    char buf[MAXSTRLEN];
    ...
    while (fgets(buf, MAXSTRLEN, f) != NULL) {
        ...
        if (strcasecmp(token, "DATA") == 0) {
            ...
            else if (now->htype != NULL)
                enkf_quit("%s, l.%d: HTYPE specified twice", fname, line);
            else
                now->htype = strdup(token);
            for (i = 0; i < ngridhtypeentries; ++i)
                if (strncasecmp(allgridhtypeentries[i].tag, token, strlen(allgridhtypeentries[i].tag)) == 0)
                    break;
            if (i == ngridhtypeentries)
                enkf_quit("%s, l %d: HTYPE \"%s\" is unknown", fname, line, token);
            ...
            for (i = 0; i < ngridvtypeentries; ++i)
                if (strcasecmp(allgridvtypeentries[i].tag, token) == 0)
                    break;
            ...
            if (strcasecmp(now->vtype, "Z") != 0 && strcasecmp(now->vtype, "NUMERIC") != 0)
                enkf_quit("%s, l.%d: VTYPE must be set to \"Z\" or \"NUMERIC\" for entry ZCVARNAME (currently \"%s\")", fname, line, now->vtype);
            if ((token = strtok(NULL, seps)) == NULL)
        ...
        } else if (strcasecmp(token, "TRIVARNAME") == 0) {
            if ((token = strtok(NULL, seps)) == NULL)
                enkf_quit("%s, l.%d: TRIVARNAME not specified", fname, line);
            else if (now->trivarname != NULL)
                enkf_quit("%s, l.%d: TRIVARNAME specified twice", fname, line);
            else
                now->trivarname = strdup(token);
        } else if (strcasecmp(token, "TRINEIVARNAME") == 0) {
            if ((token = strtok(NULL, seps)) == NULL)
                enkf_quit("%s, l.%d: TRINEIVARNAME not specified", fname, line);
            else if (now->neivarname != NULL)
                enkf_quit("%s, l.%d: TRINEIVARNAME specified twice", fname, line);
            else
                now->neivarname = strdup(token);
        } else
    ...
    for (i = 0; i < *ngrid; ++i) {
        ...
        } else if (strcasecmp(gprm->vtype, "NUMERIC") == 0) {
            if (gprm->zvarname == NULL)
        ...
        if (gprm->trivarname != NULL) {
            int htype = gridprm_gethtype(gprm);

            if (htype == GRIDHTYPE_UNDEFINED)
                gprm->htype = strdup("UNSTRUCTURED");
            else if (htype != GRIDHTYPE_UNSTRUCTURED)
                enkf_quit("%s: grid \"%s\": TRIVARNAME can only be specified for HTYPE = UNSTRUCTURED", fname, gprm->name);
        }
        if (strcasecmp(gprm->vtype, "NONE") != 0) {
            ...
            if (gprm->nzints == 0) {
                if (eprm->nzints == 0) {
                    ...
                    gprm->nzints = eprm->nzints;
                    gprm->zints = malloc(gprm->nzints * sizeof(zint));
                    memcpy(gprm->zints, eprm->zints, gprm->nzints * sizeof(zint));
                }
...
void gridprm_destroy(int ngrid, gridprm prm[])
{
    ...
    for (i = 0; i < ngrid; ++i) {
        ...
        if (now->nzints > 0)
            free(now->zints);
        if (now->trivarname != NULL)
            free(now->trivarname);
        if (now->neivarname != NULL)
            free(now->neivarname);
        free(now->domainname);
    }
    free(prm);
...
void gridprm_print(gridprm* prm, char offset[])
{
    ...
    } else if (strcasecmp(prm->vtype, "NUMERIC") == 0) {
        enkf_printf("%s  ZVARNAME = \"%s\"\n", offset, prm->zvarname);
        if (prm->zcvarname != NULL)
            enkf_printf("%s  ZCVARNAME = \"%s\"\n", offset, prm->zcvarname);
        else
    ...
    if (prm->vdirection != NULL)
        enkf_printf("%s  VDIR = \"%s\"\n", offset, prm->vdirection);
    if (prm->htype != NULL)
        enkf_printf("%s  HTYPE = \"%s\"\n", offset, prm->htype);
    if (prm->xvarname != NULL) {
        enkf_printf("%s  XVARNAME = \"%s\"\n", offset, prm->xvarname);
        enkf_printf("%s  YVARNAME = \"%s\"\n", offset, prm->yvarname);
    }
    if (prm->trivarname != NULL) {
        enkf_printf("%s  TRIVARNAME = \"%s\"\n", offset, prm->trivarname);
        enkf_printf("%s  TRINEIVARNAME = \"%s\"\n", offset, prm->neivarname);
    }
...
int gridprm_getvtype(gridprm* prm)
{
    ...
    for (i = 0; i < ngridvtypeentries; ++i) {
        if (strcasecmp(prm->vtype, allgridvtypeentries[i].tag) == 0)
            return allgridvtypeentries[i].type;
    }

    if (i == ngridvtypeentries)
        enkf_quit("%s: grid \"%s\": VTYPE \"%s\" is unknown", prm->prmfname, prm->name, prm->vtype);
...
int gridprm_gethtype(gridprm* prm)
{
    ...
    if (prm->htype == NULL)
        return GRIDHTYPE_UNDEFINED;

    for (i = 0; i < ngridhtypeentries; ++i) {
        if (strcasecmp(prm->htype, allgridhtypeentries[i].tag) == 0)
            return allgridhtypeentries[i].type;
    }

    if (i == ngridhtypeentries)
        enkf_quit("%s: grid \"%s\": HTYPE \"%s\" is unknown", prm->prmfname, prm->name, prm->htype);
