...
void grid_describeprm(void)
{
    ...
    enkf_printf("    VTYPE            = { z | zt | sigma | hybrid | numeric | none }\n");
    enkf_printf("  [ VDIR             = { fromsurf* | tosurf } ]\n");
    enkf_printf("  [ GEOGRAPHIC       = { 0 | 1* | 2 } ]\n");
...
int grid_isstructured(grid* g)
{
    if (g->hgrid->type == GRIDHTYPE_RECTANGULAR || g->hgrid->type == GRIDHTYPE_CURVILINEAR || g->hgrid->type == GRIDHTYPE_CURVILINEAR2 || g->hgrid->type == GRIDHTYPE_2D || g->hgrid->type == GRIDHTYPE_NONE) {
        ...
        return 1;
    } else if (g->hgrid->type == GRIDHTYPE_UNSTRUCTURED || g->hgrid->type == GRIDHTYPE_1D) {
        assert(g->hgrid->nj <= 0);
